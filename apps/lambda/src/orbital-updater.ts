...
function getInvocationTime(event: ScheduledEvent): Date {
...
async function findOldestBodies(
  invocationTime: Date,
  ...
  const bodies = client
    .db()
    .collection<OrbitalBody>(options.collectionName);
  ...
  const oldestBodies = await bodies
    .aggregate<WithId<OrbitalBody>>([
      ...initialStages,
      { $sort: { updatedAt: 1 } },
      { $limit: BODY_BATCH_SIZE },
    ])
    .toArray();
...
export async function updateOrbitalBodies(
  ...
  const invocationTime = getInvocationTime(event);
